...
interface BasicTableOneProps {
  data: any[];
  columns: string[];
  originalCsv: string; // ✅ Ajouter le CSV original
  predictionFile: File;
  actualFile: File;
}

export default function CompareTableCsv({ 
  data, 
  columns, 
  originalCsv, // ✅ Recevoir le CSV original
  predictionFile,
  actualFile 
}: BasicTableOneProps) {
  ...
  //   // ✅ Télécharger le CSV original avec métadonnées
  //   predictionService.downloadOriginalCompareCSV(originalCsv, "compare_results.csv");
  // };

  // Alternative: Téléchargement direct depuis le backend
  const downloadResultsFromBackend = async () => {
    try {
      await predictionService.downloadCompareCSV(predictionFile, actualFile);
    } catch (error) {
 ...
 {/* Header 
          <button 
            onClick={downloadResults} // ✅ Utilise le CSV original
            className="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200"
          >
            <svg
...
*/}
          {/* ✅ Bouton alternatif pour téléchargement direct */}
          <button 
            onClick={downloadResultsFromBackend}
            className="inline-flex items-center gap-2 rounded-lg border border-blue-300 bg-blue-50 px-4 py-2.5 text-sm font-medium text-blue-700 shadow-sm hover:bg-blue-100 hover:text-blue-800 dark:border-blue-700 dark:bg-blue-900/20 dark:text-blue-400 dark:hover:bg-blue-900/30"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="w-5 h-5"
