...
const DropZoneCSVComparePrediction: React.FC<DropzoneCSVComponentProps> = ({ onFilesAccepted }) => {
  const [files, setFiles] = useState<File[]>([]);

  const onDrop = (acceptedFiles: File[]) => {
    setFiles(acceptedFiles);
    onFilesAccepted(acceptedFiles); // ✅ envoyer au parent
  };
            ...
            {files.length > 0 ? (
              ...
              <>
                <h4 className="mb-3 font-semibold text-gray-800 dark:text-white/90">
                  {isDragActive ? "Drop CSV File with Predictions Here" : "Drag & Drop CSV File with Predictions Here"}
                </h4>
                <span className="text-center mb-5 block w-full max-w-[290px] text-sm text-gray-700 dark:text-gray-400">
                  Drag and drop your CSV file containing the column "Prediction_Rendement" 
                </span>
...
export default DropZoneCSVComparePrediction;
